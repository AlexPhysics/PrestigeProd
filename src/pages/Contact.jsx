...
const Contact = () => {
  ...
  useEffect(() => {
    ...
    const loadCalendly = () => {
      ...
      // Fallback timer - force hide after 8 seconds no matter what
      const forceHideTimer = setTimeout(() => {
        console.warn('Calendly loading timeout - hiding overlay');
        hideLoadingOverlay();
      }, 8000);
      ...
      script.onload = () => {
        ...
        const checkCalendlyReady = () => {
          ...
          // Strategy 1: Check if iframe exists and has src
          if (iframe && iframe.src) {
            console.log('Calendly iframe detected');
            clearTimeout(forceHideTimer);
            
            // Strategy 2: Listen for iframe load
            iframe.onload = () => {
              console.log('Calendly iframe loaded');
              hideLoadingOverlay();
            };
            
            // Strategy 3: Fallback timer for iframe load
            setTimeout(() => {
              if (document.querySelector('.calendly-loading')?.style.opacity !== '0') {
                console.log('Calendly iframe fallback timeout');
                hideLoadingOverlay();
              }
            }, 4000);
          ...
          // Strategy 4: Check if Calendly API is available
          if (window.Calendly && window.Calendly.initInlineWidget) {
            console.log('Calendly API available');
            clearTimeout(forceHideTimer);
          ...
          if (attempts < maxAttempts) {
            setTimeout(checkCalendlyReady, 500);
          } else {
            console.warn('Calendly detection failed after max attempts');
            clearTimeout(forceHideTimer);
            hideLoadingOverlay();
          }
        };
        ...
        setTimeout(checkCalendlyReady, 1000);
      };
    ...
    // Load Calendly after a short delay to prioritize main content
    const timeoutId = setTimeout(loadCalendly, 1000);
          ...
          >
            ...
            {status === 'ERROR' && (
              ...
              >
                ...
                <p className='text-white/60 text-sm mt-1'>
                  {t('contact.errorSubtext', 'Or contact us directly at info@prestigeproduction.ch')}
                </p>
              </div>
            )}
            ...
            <div className='relative group fade-in'>
              <button
                ...
                style={isSubmitting || Object.keys(errors).length > 0 ? {} : {
                  background: 'linear-gradient(135deg, #205C57, #9EB6A9)'
                }}
              ...
              <div id="submit-status" className="sr-only" aria-live="polite">
                {isSubmitting ? t('contact.submittingStatus', 'Form is being submitted') : ''}
              </div>
            </div>
          </form>
        )}
        ...
        <div className='mt-20 border-t border-pp-grey/30 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4'>
          <div className='text-sm space-y-1 text-pp-grey'>
            <p>
              ...
              <a href='mailto:info@prestigeproduction.ch' className='underline hover:text-pp-teal transition-colors'>
                info@prestigeproduction.ch
              </a>
            </p>
            <p>
              {t('contact.phone', 'Phone')}:{' '}
              <a href='tel:+41762021959' className='underline hover:text-pp-teal transition-colors'>
                +41 76 202 19 59
              </a>
            </p>
          </div>
